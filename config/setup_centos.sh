...
export CLONE_PATH=/usr/local/src
export APP_PATH=/usr/local/app
...
sudo dnf -y install libgcc.i686 glibc-devel bison flex texinfo libtool \
...
sudo dnf -y --skip-broken install libpng libpng-devel libjpeg-devel ghostscript-devel libtiff-devel libwmf-devel \
        protobuf-devel mesa-dri-drivers coreutils
sudo dnf -y groupinstall "Development Tools"
sudo dnf -y install git sudo gcc gcc-c++ gdb make unzip ctags vim expect passwd wget cmake figlet ncdu \
        nnn gh kakoune colordiff git-lfs the_silver_searcher ack fortune-mod tree ninja-build pkgconfig \
        curl file
sudo dnf -y install perl nodejs rust python2 python2-pip python2-devel \
        python3 python3-pip python3-devel ruby lua luajit zsh
sudo dnf -y remove mesa-libGL mesa-libGL-devel
sudo ln -s /usr/bin/python3 /usr/bin/python
sudo ln -s /usr/bin/pip3 /usr/bin/pip
sudo sed -i '/Defaults/s/env_reset/\!env_reset/g' /etc/sudoers
sudo mkdir ${APP_PATH}
sudo chmod a+w ${CLONE_PATH} ${APP_PATH} ${BIN_PATH}
sudo ln -sf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime

git clone https://github.com/ImageMagick/ImageMagick ${CLONE_PATH}/ImageMagick \
...
git clone https://github.com/so-fancy/diff-so-fancy ${APP_PATH}/diff-so-fancy \
...
git clone https://github.com/hackerb9/lsix ${APP_PATH}/lsix \
&& ln -s ${APP_PATH}/lsix ${BIN_PATH}/lsix

git clone https://github.com/facebook/PathPicker ${APP_PATH}/PathPicker \
...
wget https://github.com/sharkdp/insect/releases/download/v5.3.0/insect-linux-x64 -O ${APP_PATH}/insect-linux-x64 \
...
git clone https://github.com/bobthecow/git-flow-completion.git ${APP_PATH}/git-flow-completion

git clone https://github.com/arzzen/git-quick-stats.git ${CLONE_PATH}/git-quick-stats \
...
git clone https://github.com/brendangregg/FlameGraph.git ${APP_PATH}/FlameGraph

git clone https://github.com/libsdl-org/SDL.git ${CLONE_PATH}/sdl \
&& cd ${CLONE_PATH}/sdl \
...
git clone https://github.com/neovim/neovim.git ${CLONE_PATH}/neovim \
&& cd ${CLONE_PATH}/neovim \
&& git checkout stable \
&& make CMAKE_BUILD_TYPE=RelWithDebInfo CMAKE_EXTRA_FLAGS="-DCMAKE_INSTALL_PREFIX=${APP_PATH}/neovim" \
&& make install

git clone https://github.com/vim/vim.git ${CLONE_PATH}/vim \
&& cd ${CLONE_PATH}/vim \
...
git clone https://github.com/mobile-shell/mosh ${CLONE_PATH}/mosh \
&& cd ${CLONE_PATH}/mosh \
&& ./autogen.sh \
...
#nodejs
sudo npm install -g fx fx-completion cloc svg-term-cli zx @squoosh/cli bash-language-server
...
#go
wget https://dl.google.com/go/go1.18.linux-amd64.tar.gz -O ${CLONE_PATH}/go1.18.linux-amd64.tar.gz \
...
git clone https://github.com/adwpc/xvim.git ${CLONE_PATH}/xvim \
&& cd ${CLONE_PATH}/xvim \
&& yes | ./install vimrc
...
git clone https://github.com/kevinschoon/pomo.git ${CLONE_PATH}/pomo \
&& cd ${CLONE_PATH}/pomo \
&& make

curl -L https://bit.ly/glances | /bin/bash

git clone https://github.com/charmbracelet/glow.git ${CLONE_PATH}/glow \
&& cd ${CLONE_PATH}/glow \
&& go build

git clone https://github.com/rs/curlie.git ${CLONE_PATH}/curlie \
&& cd ${CLONE_PATH}/curlie \
&& go install

git clone https://github.com/muesli/duf.git ${CLONE_PATH}/duf \
&& cd ${CLONE_PATH}/duf \
&& go install

git clone https://github.com/multiprocessio/dsq ${CLONE_PATH}/dsq \
&& cd ${CLONE_PATH}/dsq \
&& go install .

git clone https://github.com/rupa/z.git ${APP_PATH}/z
